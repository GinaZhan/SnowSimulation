...
def setup_simulation():
    ...
    velocities = wp.zeros((num_particles, 3), dtype=wp.vec3, device="cuda")
    ...
    for i in range(num_particles):
        ...
        pos[1] += 0.2
        pos[2] += 11
        positions[i] = wp.vec3(*pos)
        velocities[i] = wp.vec3(0.0, 0.0, 0.0)

    particle_system.positions = positions
    particle_system.velocities = velocities
...
particle_positions = solver.particle_system.positions.to_numpy()
